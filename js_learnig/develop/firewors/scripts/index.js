...
var canvas = document.getElementById( "canvas" ),
  ...
  mouseX = 0,
  mouseY = 0,
  iconList = document.getElementById( "icon_list" );
...
FireWorks.prototype.createFire = function () {
  for (var i = 0; i < number; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 11, this.speed, "106, 255, 106"));
  }
  for (var i = 0; i < 24; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 15 + 5, this.speed * 0.84, "106, 255, 106"));
  }
  for (var i = 0; i < 18; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 20, this.speed * 0.68, "106, 106, 255"));
  }
  for (var i = 0; i < 15; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 24 + 9, this.speed * 0.52, "106, 106, 255"));
  }
  for (var i = 0; i < 12; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 30, this.speed * 0.36, "106, 255, 106"));
  }
  for (var i = 0; i < 9; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, i * 40 + 3, this.speed * 0.20, "106, 255, 106"));
  }
  for (var i = 0; i < 1; i++) {
    this.fires.push(new FireWork(this.ctx, this.x, this.y, 0, 0, "106, 106, 255"));
  }
};
    ...
    for (var i = 0; i <= temp.height * 4; i += 4) {
      for (var j = 0; j <= temp.width * 4; j += 4) {
        if (j % 40 == 0 && i % 40 == 0) {
          ...
          var relativeX = j / 2 - temp.width;
          var relativeY = i / 2 - temp.height;
          // var angle = Math.atan2(i / 4 - temp.height / 8, -(j / 4 - temp.widt / 8)) * 180 / Math.PI;
          ...
          var speed = getDistance(Math.abs(self.x + relativeX), self.x,
           Math.abs(self.y + relativeY), self.y) / 30;
